...
>
> **行号快照**:本文核对时的规模为 `main.rs` 177、`app.rs` 5216、`engine.rs` 2030、`keymap.rs` 1139、`capture.rs` 587、`adb.rs` 708、`theme.rs` 679、`control.rs` 243、`filedialog.rs` 81、`settings.rs` 479 行。**计数口径**:`Get-ChildItem src\*.rs | ForEach-Object { (Get-Content -Encoding UTF8 $_.FullName).Count }` —— `-Encoding UTF8` 不能省:这些文件是**无 BOM 的 LF 文本**,Windows PowerShell 5.1 的 `Get-Content` 默认按 ANSI 解码,"某个汉字的末字节恰好是 GBK 前导字节 + 紧随的换行"会被吞掉一个换行,于是同一个文件能少算几十行(`settings.rs` 431 vs 479、`文档\功能组分说明.md` 716 vs 882)。这些文件在写作期间正被连续修改(接线陆续落地),因此行号仅作定位参考,**关键处优先给函数名**。
>
> **本文已计入的三批改动**:① 第一批:`Wheel.scope`、`settings.rs` 及其接线、引擎的"关闭映射收尾"重构;② 第二批(2026-09-21):引擎的**共享触点池 + 物理按键镜像 + 归属对账 + 结构指纹**(§1.3 / §3.6 / §3.9)、`settings.rs` 的 **scrcpy 目录与 BOM 容错**(§1.3 / §3.2 / §7.2)、浮层的**键位显示亮度与画布坐标空间**(§1.3 / §3.2b / §7.4);③ **第三批(2026-09-21 续,即"第二轮")**:浮层可读性三招 + `LumaGrid` 逐处取档(§1.3 / §3.2b / §3.10 / §7.5)、摇杆标注瘦身与信息卡(§1.3 / §3.10 / §7.5)、引擎**上升沿 / 通知 / 坐标空间重查**(§1.3 / §3.6 / §7.5)。单测数量按 `#[test]` 实数更新为 **43**(§8)。
...
| `app.rs` | 5216 | 界面与"会话指挥":egui 全量 UI、启动序列、连接编排、配置/外观/设置的读写、取点与浮层绘制(含**亮度网格与摇杆信息卡**)、引擎通知与坐标空间重查的界面侧 | `adb`、`capture`、`control`、`engine`、`keymap`、`theme`、`filedialog`、`settings` | `main.rs`(`app::PadApp::new`);eframe 事件循环回调 `eframe::App::ui` |
| `engine.rs` | 2030 | 映射引擎线程:消费捕获事件、优先级判定(**总开关键与切换模式启用键只认上升沿**)、定时动作队列、瞄准子系统、**共享触点池预算与归属对账**、把命令推进控制通道;并提供两条"关闭映射"路径共用的收尾实现,以及**引擎→界面的通知与坐标空间重查请求** | `capture`、`control`、`keymap` | `app::PadApp::new` 里 `std::thread::spawn(\|\| engine::run(...))` |
...
| `theme.rs` | 679 | 语义色、尺寸常量、配色/密度/背景图预设、**浮层可读性三招(描边外套 `casing` / 衬底 `plate` / 自动对比 `auto_tone`)**,以及 `apply_style` 把外观刷进 egui 样式 | 无(叶子) | `app`、`keymap`(`Profile.look` 字段类型) |
...
- `PadApp` 私有方法(按用途分组):
  ...
  取值:`mapper` / `theme` / `look` / **`wheel_at`**(画布命中测试:截图坐标落在哪个摇杆圈里)
...
- 自由函数:`profile_path` / `look_cache_path` / `load_look_cache` / `load_profile` / `read_profile_at` / **`read_config_text`** / **`backup_broken_config`** / **`locate_remembered_scrcpy`** / `write_default_profile` / `fmt_timestamp` / `timestamp_compact` / `civil_from_days` / `set_circle_angle` / `circle_geometry` / `is_draft_slot` / `swipe_controls` / `draw_swipe_track` / **`draw_wheel_card`**(摇杆信息卡) / `draw_easing_preview` / `install_cjk_font` / `egui_key_code`
- 私有类型:`StartPreset`、`ArgHelp`、`ArgEntry`、`KeySlot`、`CoordSlot`、`OverlayFilter`、`EasingEditTarget`、`DialogPurpose`(**含 `ScrcpyDir`**)、`DraftBind`(含 `Default`)、**`LumaGrid`**(截图的局部亮度网格:`new(&ColorImage)` / `around(x, y)`)
- 常量:`SCID = 0x1a2b3c4d`、`LOCAL_PORT = 28383`、`REPO_URL`、`AUTHOR`、`LICENSE_TEXT`、`BASE_SCRCPY_ARGS = "--stay-awake"`、`COORD_RANGE = -8192..=8192`、**`WHEEL_CLICK_MIN_RADIUS = 24.0`**(摇杆信息卡的最小命中半径,截图像素)
...
- `pub struct Shared { pub profile: Profile, pub enabled: bool, pub control: Option<ControlClient>, pub aim_live: AimLive, pub live: EngineLive, pub notices: Vec<String>, pub space_recheck: bool, pub toolbar_release: bool }` —— `toolbar_release` 是"顶栏[映射: 开/关]按钮请求一次关闭收尾"的**请求位**(见 §3.6 1b 与 §3.9),`live` 供左栏自检显示;`notices` 是**引擎→界面的一句话通知队列**(上限 16 条,界面每帧 `take` 走),`space_recheck` 是"请界面重新确认一次触摸坐标空间"的请求位(见 §3.6 第 8c 步)
...
- 私有函数:`bind_pid` / `wheel_pid` / `cancel_up_for` / `bind_point` / `point_at_progress` / `deactivate_wheel` / `release_conflicting_binds` / `update_wheel` / **`reconcile_binds`** / **`reconcile_wheels`** / **`key_owned_by_wheel`** / **`wheel_pointers`** / **`refuse`** / **`notify`** / **`rising_edge`** / **`release_all_binds`** / **`sync_structures`** / **`binds_signature`** / **`wheel_signature`** / `aim_active` / `aim_lift` / `aim_release_local` / `aim_anchor` / `aim_target` / `aim_recenter` / `aim_on_motion` / `aim_tick` / `is_ctrl` / `is_alt` / `ctrl_alt_chord`
...
- `pub enum Preset` / `Density` / `BgFit`;`pub struct Look`(`theme`/`has_bg`/`clear_bg`,字段含 **`overlay_tone: f32`** 与 **`overlay_auto: bool`**,后者 `#[serde(default = "default_true")]`)
- `pub fn apply_style`、`with_alpha`
- **浮层亮度档位**:`pub const TONE_MIN = -1.0` / `TONE_MAX = 1.0`、`clamp_tone`、`tone_color`、`tone_alpha`、`tone_text`、`tone_ring_fill`
- **浮层可读性(第二轮)**:`pub fn auto_tone(bg_luma, manual)`(背景亮度 → 自动档位,叠加手动微调)、`pub fn luma(color)`(感知亮度)、`pub fn casing(ink)`(与本体反相的描边外套色)、`pub fn plate(ink)`(文字衬底色)、`pub fn paint_text(...)`(只描边不垫底)、`pub fn paint_label(...)`(衬底 + 描边 + 正文)
- 已删除:`outline_text` / `outline_stroke`(恒为白色,引入亮度档位后无调用点,原地留了说明注释,`theme.rs:398-401`)
...
| 7 | 回收截图 | `shot_rx.try_recv()` → **先建亮度网格 `shot_lum = LumaGrid::new(&img)`** → `ctx.load_texture` → `shot = Some((tex,w,h))` → **清掉已点开的摇杆信息卡 `wheel_info = None`** → `sync_display_space(w,h)` | 截图尺寸就是"当前屏幕方向"下的真实坐标空间,顺手校正;亮度网格必须在图还活着时建好(浮层自动对比要用),而**换了截图之后旧的信息卡指向的是另一张图**;**浮层绘制也因此优先用截图尺寸**(见 §3.10) |
...
| 10b | **收引擎的通知与坐标空间重查请求**(第二轮) | 一次 `lock` 里 `std::mem::take(&mut g.notices)` + `take(&mut g.space_recheck)`,锁外逐条 `self.log(m)`;若 `space_recheck` 为真则 `self.space_rx = None; self.refresh_display_space()` | 引擎线程没有日志所有权,只能把"映射已开启/已关闭、触点池满放弃了哪个键、配置结构变化导致抬手"放进 `notices`,由界面每帧取走;**先 `take` 出锁外再打日志**,是因为 `log()` 只碰 `self.logs`、但持锁时调它可以避免任何"表达式里二次加锁"的风险(§3.5 约定 3)。坐标空间重查同理:引擎只负责"请求",查询与写回仍在界面线程 |
| 11 | 同步标志与状态 | `grab_flag.store(enabled && grab_enabled)`;由关到开且已连接时 `refresh_display_space()`;回收 `space_rx`;鼠标捕获状态变化时记一条日志;`server.is_some() && !connected && connect_rx.is_none()` → 判为断线并清空 | `mouse_grab` 由引擎维护,这里只读来显示;grab 只管键盘。**第二轮后"由关到开"不再是唯一的自动重查入口**:第 10b 步的引擎请求走的是同一条 `refresh_display_space()` |
...
| 13 | 左栏 `egui::Panel::left("left")` | 上部**可滚动的配置区**:"使用说明"折叠、捕获错误红字、**引擎状态自检**(`引擎: 触点 n/10 · 因触点池满被放弃 N 次(最近是哪个键)`,读 `Shared.live`)、[保存配置]/[另存为]/[重新加载]/[选用配置]/[新建配置]/[检测当前 json]/当前配置路径/[保存日志]、"外观"折叠(`ui_look`,含 **[键位自动对比] 复选框**与**"键位显示亮度(微调)"滑块**)、总开关键 + grab 复选框、"scrcpy 管理"(**scrcpy 目录输入框 + [浏览目录] + [应用]**、[自动寻找全部]/[测试并刷新]、[记住路径] / [打开配置目录]、三个路径输入框 + [浏览]/[自动]、当前 adb 与 server 版本);下部日志区(`stick_to_bottom`,固定高度,倒序取最近 200 条) | 配置区高度用 `available_height() - 150` 预留日志区,日志因此不会被顶出屏幕;引擎状态放在最前面,因为"按了没反应"是最高频的疑问。**第二轮之后,这一栏的日志区同时是"引擎通知"的落点**(第 10b 步):映射开关、触点池满、配置重建这三类事件都能在这里看到一句话 |
...
| `SharedState` | `Arc<Mutex<Shared>>` | 双向 | `profile`(UI 写、引擎只读并 clone 小结构)、`enabled`(引擎翻转、UI 显示与可点击)、`control`(UI 写、引擎借用)、`aim_live`(引擎写、UI 读来诊断显示)、`live: EngineLive`(引擎写、UI 读来显示"触点 n/10 · 被放弃 N 次")、**`notices: Vec<String>`**(引擎写、UI 每帧 `take` 后写进日志;§3.6 ⑦)、**`space_recheck: bool`**(引擎置位请求"重新确认坐标空间"、UI `take` 后跑 `refresh_display_space()`;§3.6 第 8c 步),`toolbar_release`(UI 置位请求关闭收尾、引擎取走并复位) |
...
sequenceDiagram
    ...
    E->>E: 顶部先清到期动作(整批一次加锁)
    E->>SH: 顶部统一落账:通知入队 / space_recheck 置位
    E->>E: 读 enabled / toggle_key / aim 门控;Ctrl+Alt 组合判定
    E->>Q: 若为按下 → gui_tx 转发(供 UI 键位捕获)
    E->>E: held.set(code, pressed) + rising_edge → fresh_press
    E->>E: 空闲 ≥3 秒后的第一批按键 → pending_space_recheck
    E->>E: 总开关键(仅上升沿) / 临时轮盘启用键(Toggle 仅上升沿) / 轮盘方向键 / 普通绑定
    E->>SH: notify: 映射已开/关、触点池满、配置重建(攒到下一轮顶部落账)
    ...
    SH-->>E: 界面下一帧 take 走 notices / space_recheck
```
...
2. **瞄准维护 + 同步鼠标捕获 + 刷新诊断快照 + 落账待办消息**:在锁内 clone 一份 `Aim`(小结构),算 `mouse_grab` 的组合布尔值并 `store`;在线且已连接时 `aim_tick`(映射关/门控松 → 抬指;静止归中 → 回锚点),否则 `aim_release_local` 只清本地状态;把 `aim.ox/oy/down/active/sent` 抄进 `Shared.aim_live`;**同时重算 `live.pointers = fingers.count + wheel_pointers(&wheels) + usize::from(aim.down)` 并写回 `Shared.live`**——界面左栏那行自检读的就是它。**第二轮在这里多做了两件事**(`engine.rs:964-972`):把上一轮攒下的 `pending_notice` 经 `notify(&mut s.notices, msg)` 落进通知队列,并把 `pending_space_recheck` 写成 `s.space_recheck = true`。**为什么攒到循环顶部才落账**:处理事件时常常正借着 `shared.control` / `shared.profile`,而 `MutexGuard` 不能整结构再可变借用一次;"借一个字段"是允许的,所以 `notify` 的参数是 `&mut Vec<String>`。
...
8b. **写物理按键镜像 + 算上升沿**:`let fresh_press = rising_edge(ev.pressed, held.has(ev.code)); held.set(ev.code, ev.pressed);`(`engine.rs:1071-1072`)。**位置在总开关键判定之前、且不受 `enabled` 与配置影响**——它是后面所有对账的唯一依据,漏掉这一次写入就等于"引擎以为用户没按这个键";`fresh_press` 则回答"这一次事件是不是这个键**刚被按下**",凡"按一下翻一次"的判定都用它。
8c. **空闲后请求重查坐标空间**(第二轮,`engine.rs:1074-1086`):用 `last_key_at` 判断"距离上一次按键是否 ≥3 秒",是则把 `pending_space_recheck` 置位(下一轮循环顶部写成 `Shared.space_recheck`,由界面执行 `space_rx = None; refresh_display_space()`)。为什么放在按键路径上:用户真实操作就是"切到别的窗口待一会儿,回来接着打"——这段时间手机可能已经转过屏,而坐标空间以前只在连接/截图/关开映射时刷新。
9. **总开关键(最高优先级,任何时候都生效,但只认上升沿)**:**第二轮起判定条件从 `ev.pressed` 改成 `fresh_press`**(`engine.rs:1088-1089`)——Windows 的按键自动重复会以 KeyPress 反复上报,若每个事件都翻转,长按 F8 半秒就能翻十几次,用户看到的是"按了没反应 / 状态跟自己以为的不一样"。翻转 `enabled` 的同时**留一条通知**(`notify(&mut g.notices, "映射已开启(总开关键 <键名>)")` / `"映射已关闭并抬起全部触点(<键名>)"`,键名由 `key_name` 现场取,`engine.rs:1094-1101`);**关闭瞬间**走与顶栏按钮完全相同的 `release_all`(`EngineState` 借用同一批状态):`aim_lift` 抬瞄准触点、遍历所有绑定 `fingers.release` 并按 `bind_point` 抬触点、`active_android_keys.drain()` 补发 `key(false)`、落下的轮盘先 `touch_move` 回圆心再 `touch_up`,最后 `fingers.free_all()` + 所有轮盘状态(含临时轮盘的 `active`)清零;没有控制通道时走 `None` 分支,只清本地状态(否则重开打时状态会残留)。**重新开启**时除了把 `aim.released` 复位,还要 `sync_structures` + `reconcile_binds` + `reconcile_wheels` **按物理按键状态把此刻仍按着的键补回来**(`engine.rs:1122-1164`)——这正是"关映射时全部触点被抬起,开映射后按着不放的键立刻生效"的实现。若此刻按钮也正好请求了收尾,则视为已一并处理(`toolbar_release = false`),避免下一轮再收一次。随后 `continue`。
10. **未开启 / 未连接直接跳过**:`if !enabled { continue }`;`let Some(ctl) = g.control.as_ref() else { continue }`;`if !ctl.is_connected() { continue }`。
11. **取换算器与结构对齐**:`let m = profile.mapper((ctl.screen_w, ctl.screen_h))` 是**唯一换算入口**;随后 `sync_structures(...)`(见 ②b)负责"结构变了先抬手再重建",再刷一次 `live.pointers`。这里不再有"轮盘数量变化 → `vec![默认值]`"的旧写法(它会漏抬触点)。**第二轮补充**:若这次真的发生了结构重建、且重建前确有触点按着(`held_before > 0`),就攒一句 `pending_notice`("配置已改动: 先抬起正在按着的触点,再按新配置重建(不会卡键)",`engine.rs:1204-1208`)——这句话解释了玩家"改配置瞬间手感跳了一下"的来由。
12. **临时轮盘启用键**:命中 `w.temp.key` 时,`Hold` 模式按下即 `active = true` 并 `release_conflicting_binds`(把与该轮盘四个方向键同键位的普通绑定/系统键先释放,免得方向输入被抢占、触点卡死),松开即 `deactivate_wheel`;**`Toggle` 模式只在 `fresh_press` 为真时翻转**(`engine.rs:1240-1251`,同样是为了不被自动重复连翻)。**只要 `active` 真的变了,就立刻 `reconcile_binds` + `reconcile_wheels` 对账一次**(`engine.rs:1269-1293`):启用方向把已按着的方向键补进摇杆,停用方向把仍按着的 `Hold`/系统键补回普通绑定。**命中即 `consumed = true` 并 `continue`** —— 启用键不会同时触发自己的其它绑定。
...
14. **普通绑定**:遍历 `profile.binds`,匹配 `bind.key == ev.code` 后按动作类型分派(下一节);申请触点时 `reserved = wheel_pointers(&wheels) + usize::from(aim.down)`,挤不进去就 `refuse(&mut live, bind.key)` 并记下这个键;末尾刷新 `live.pointers`,并把"触点池已满、某某键这次按下被放弃、松掉几个键即可"攒成一条通知(**每秒最多一条**,`engine.rs:1445-1458`)。

**②b 结构指纹与释放路径(第二批新增,专治"触点越攒越多")**

```mermaid
flowchart TD
    SYNC["sync_structures(每轮、每次事件都会调)"] --> BS{"binds_signature 变了?"}
    ...
    RB -.->|"重建前有触点按着 → 攒一条通知<br/>(第三批: notify)"| NOTE["Shared.notices:<br/>『配置已改动: 先抬起正在按着的触点…』"]
```

- **指纹只认结构**:`binds_signature` = 数量 + 每个键的键码 + 动作类型(`Tap` 还区分 `duration_ms == 0` 与否);`wheel_signature` = 数量 + 四个方向键 + 启用键与模式(`engine.rs:441-459`、`470-488`)。**只改坐标、半径、`scope` 不触发重建**——否则玩家一边调参一边打会被反复抬手。单测 `signatures_track_structure_not_geometry`(`engine.rs:1855`)锁死这条边界。
- **释放路径必须遍历"按下表本身"**:`release_all_binds`(`engine.rs:347-365`)取 `n = fingers.down.len().max(profile.binds.len())`,因为用户删掉的槽位里可能还记着"按着";越界槽位用 `(0,0)` 抬起——坐标一定在屏内,不会因"落在屏外"被系统整条丢弃。
- **轮盘先回中再抬**:旧圆心若在新配置里已经不存在,就用 `st.last` 兜底(`engine.rs:789-794`),同样是为了给出一个屏内坐标。
- **为什么必须这样**:索引位移之后,旧触点的"已按下"记录会被错记到别的键上(那个键于是"按了没反应"),而真正的旧触点永久留在设备上——触点池越用越少,最后所有按键都没反应。修法就是"宁可抬手重算,绝不带着旧状态跑"。
- **第三批补上的一句话**:重建本身对玩家是"无感"的(§3.6 第 11 步会立刻对账),但"手感跳了一下"是真实存在的;所以在"重建前确有触点按着"时留一条通知,让这件事从"玄学抖动"变成"日志里有解释的抬手"(`engine.rs:1204-1208`)。
...
- 首次按下:先看 `reserved >= DEVICE_MAX_POINTERS`,池满则**不注入**并返回 `true`(调用方 `refuse` 计数,并在第三批之后额外留一条限流通知),否则 `touch_down(圆心)` 然后 `touch_move(目标)`(先落圆心再推,符合摇杆手感);
...
**⑦ 引擎→界面的两条"一次性信箱"**(第三批新增)

| 字段 | 谁写 | 谁读 / 怎么消费 | 语义 |
|---|---|---|---|
| `Shared.notices: Vec<String>` | 引擎(`notify(&mut Vec<String>, msg)`,上限 16 条) | 界面每帧 `std::mem::take` 出锁外,逐条 `log()`(`app.rs:1906-1919`) | "给你一句解释":映射被开/关、触点池满放弃了哪个键、配置重建抬了手。**它不是日志系统**,真正的日志与 200 条上限仍在界面侧 |
| `Shared.space_recheck: bool` | 引擎(空闲 ≥3 秒后的第一批按键) | 界面每帧 `take`,为真则 `space_rx = None; refresh_display_space()` | "请重新确认一次触摸坐标空间"。引擎只请求,查询(adb)与写回仍在界面线程 |

- **为什么用"信箱"而不是回调**:引擎与界面之间只有 `Shared` 一把锁、两条 channel;`gui_tx` 那条通道的语义是"按下的按键"(供绑定捕获),不能混入别的东西。加两个字段是最小改动,也天然满足"界面每帧轮询"的现有节奏。
- **为什么引擎侧要攒一下再写**:见第 2 步(`pending_notice` / `pending_space_recheck`)——事件处理路径上常常正借着 `shared.control` / `shared.profile`。
...
| 名字 | 取自 | 用途 | 代码位置 |
|---|---|---|---|
| 画布空间 | **优先截图尺寸**(`shot`),取不到才退回控制通道尺寸,再取不到返回 `None` | 浮层绘制、取点换算、草稿初始化、像素值显示、**摇杆信息卡命中判定** | `PadApp::screen_size`(`app.rs:3702-3715`) |
| 注入空间 | 控制通道的 `screen_w/screen_h` | 引擎把相对坐标换算成设备像素、注入消息里的 `screen_size` 字段 | `PadApp::inject_space`(`app.rs:3717-3725`) |
...
flowchart TD
    ...
    SHOT --> LUM["LumaGrid::new(&img)<br/>至多 64×64 的亮度均值网格"]
    LUM --> TONE["draw_overlay 的 tone_at(sx, sy)<br/>每个标注按自己底下的像素取档位"]
    TONE --> OV
    MAPPER --> CARD["wheel_at(px, py): 命中半径 = max(半径, 24px)<br/>→ wheel_info → draw_wheel_card"]
    CARD --> OV
    ...
    SS -.->|"两者不一致时"| WARN["画布上方红字提示<br/>建议重新截图再取点<br/>app.rs:4430-4445"]
    CTL -.-> WARN
```

- **为什么画布必须优先截图**:浮层是**画在截图上**的。用会变的控制通道尺寸去画一张不变的图,观感就是"同一个摇杆毫无征兆地突然变大/移位"——这正是用户报的"创建新摇杆时旧摇杆瞬间变大"。
- **为什么缩放系数不能用 `available_height()`**:`ui_picker` 位于中央区滚动容器里,右侧多出一行轮盘设置就会让"剩余高度"变化,于是整张浮层跟着缩放。改成 `ui.ctx().content_rect().height()` 之后,窗口不变时同一张截图的缩放系数恒定(`app.rs:4450-4456`)。
...
**逐标注采样 + 外套 + 衬底(第三批:浮层可读性)**

```mermaid
flowchart TD
    T0["draw_overlay 开头(先取 Mapper,再加锁)"] --> T1["tone_manual = look.overlay_tone<br/>auto = look.overlay_auto"]
    T1 --> T2{"有 shot_lum 且 auto?"}
    T2 -->|"是"| T3["tone_at(sx, sy) =<br/>auto_tone(grid.around(sx, sy), tone_manual)"]
    T2 -->|"否"| T4["tone_at(..) = clamp_tone(tone_manual)"]
    T3 --> D1["每个键位: tone_at(自己的像素)"]
    T4 --> D1
    D1 --> D2["tone_ring_fill(ring, fill, tone)<br/>→ circle_filled 填充"]
    D2 --> D3["circle_stroke(KEY_STROKE + 2.5, casing(ring))<br/>← 反相外套"]
    D3 --> D4["circle_stroke(KEY_STROKE, ring)<br/>← 本色圈"]
    D4 --> D5["paint_label: plate 衬底 + 反相描边 + tone_text 正文"]
```

| 元素 | 档位取自 | 外套 / 衬底 | 代码位置 |
|---|---|---|---|
| 键位圈(点按/长按/改范围中) | 该键位的配置坐标 → `tone_at(px, py)` | 填充走 `tone_ring_fill`;圈先垫 `KEY_STROKE + 2.5` 的 `casing`,再画本色圈;键名走 `paint_label` | `app.rs:3208-3237` |
| 滑动轨迹 | 起点像素 | 线/框/圆走 `tone_ring_fill`,线/框/圆与起点小圈都垫 `theme::casing` 反相外套(与键位圈同一套可读性处理);标注"起点小圈 + 键名"走 `paint_label` | `app.rs:3239-3252`、`draw_swipe_track`(`app.rs:5034-5095`) |
| 新增草稿 | 草稿自己的像素 | 圈垫 `casing`;标签走 `paint_label` | `app.rs:3279-3298` |
| FPS 瞄准锚点 | 锚点像素 | 圆垫 `casing`;"瞄准锚点 [门控键]"走 `paint_label` | `app.rs:3304-3357` |
| 永久摇杆 | 圆心像素 | 圆环垫 `casing`(6.0);圆心小圈垫 `casing`(3.5);编号走 `paint_label` | `app.rs:3430-3454` |
| 临时摇杆 | 圆心像素 | 虚线环先垫一层 **5.0 反相粗虚线** 再画 2.0 本色虚线;圆心小圈同上 | `app.rs:3387-3429` |
| 影响范围外环(`scope`) | 圆心像素 | 橙色虚线同样先垫 3.5 反相粗虚线;标签走 `paint_label` | `app.rs:3455-3484` |
| 摇杆信息卡 | 该摇杆圆心像素 | 整卡垫 `theme::plate(ink)` + 1.5px 摇杆语义色边;标题与各行文字走 `theme::paint_text`(已有整块衬底,不再逐行垫) | `draw_wheel_card`(`app.rs:4944-5030`) |

- **"逐标注采样"为什么重要**:这是第一批那个"一个滑块管全部"的正面回答。`draw_overlay` 仍然只读一次配置(拿到手动档位与开关),但**每一处绘制各自调 `tone_at`**——同一次绘制里,压在亮块上的键位会得到负档位(加深)、压在暗块上的会得到正档位(变浅),彼此不再互相将就。
- **退路有两条**:没有截图(`shot_lum == None`)或用户关掉 [键位自动对比] 时,`tone_at` 一律退回 `clamp_tone(tone_manual)`——也就是第一批的行为,逐字节不变(`theme::tone_ring_fill` 在 `tone == 0` 时原样返回,单测 `tone_ring_fill_is_identity_at_zero`)。
- **`LumaGrid` 的三个尺寸概念**:网格宽高(`gw × gh`,至多 64×64)、截图尺寸(`w × h`)、查询点(`px, py`,截图像素)。`around` 先把查询点换算成格号、再取 **3×3 格**平均;越界/负坐标/退化尺寸都夹住或返回中性值 `0.5`,**绝不 panic**(单测 `luma_grid_samples_safely`,它每帧、每个标注都会被查一次)。

**摇杆信息卡(第三批)的绘制与命中**

```mermaid
flowchart LR
    CLICK["ui_picker: 点击截图"] --> P{"有 picking?"}
    P -->|"有"| PK["取点(信息卡让位)"]
    P -->|"没有"| HIT{"wheel_at(px, py) 命中?"}
    HIT -->|"命中 i"| TG{"wheel_info == Some(i)?"}
    TG -->|"是"| OFF["收起(wheel_info = None)"]
    TG -->|"否"| ON["展开(wheel_info = Some(i))"]
    HIT -->|"没命中"| BLANK["wheel_info = None<br/>并记一条『截图坐标: (x, y)』"]
    ON --> DRAW["draw_overlay: 该摇杆多描一圈细环<br/>→ draw_wheel_card"]
    OFF --> DRAW
```

- **命中判定**(`wheel_at`,`app.rs:4546-4574`):以**画布空间**换算每个摇杆的圆心与半径,命中半径取 `m.len(w.radius).max(WHEEL_CLICK_MIN_RADIUS)`(24 截图像素)——用户可能把摇杆调得很小,但"点一下看键位"这个动作不该要求点得那么准。多个摇杆重叠时**取最后绘制的那个**(循环里不 break,持续覆盖 `hit`),与眼睛看到的上下层一致。
- **卡片内容**(`draw_wheel_card`,`app.rs:4944-5030`):标题「摇杆N / 临时摇杆N」+ 四行「上/下/左/右 <键名>」;`w.temp` 存在时追加一行「启用 <键> · 按住启用/再按切换」(文案由 `TempMode` 决定)。**画布上那一长串标注就此消失**(`quick_label`,`app.rs:3379-3386` 只写编号),细节改成按需查看。
- **摆位规则**:先量出最长一行的宽度与最大行高定出卡片尺寸,默认放在圈的**右边**(`center.x + radius + 12`);右边放不下就翻到**左边**;再不够就夹在画布内边 4px 处(`app.rs:4994-5000`)。
- **配色**:卡片垫 `theme::plate(ink)`,描 1.5px 的**摇杆语义色**边(临时=品红 `wheel_temp`、永久=青 `wheel_perm`,都先过 `tone_color`),标题也用这个边色;正文用 `tone_text(tone)`——于是信息卡与画布上的圈一眼对应。
- **生命周期**:`wheel_info` 在"换截图"时清空(`app.rs:1743`);取点(`picking`)存在时点击一律走取点,信息卡不会被误触;被点开的摇杆会多描一圈 `r + 6.0` 的细环表示选中(`app.rs:3408-3414`、`3435-3441`)。
...
| 选择文件夹 | `filedialog::pick_folder`、`Mode::Folder`、`DialogPurpose::ScrcpyDir` | `filedialog.rs:20-44`、`55-81`;`app.rs:2369-2388` | §3.2b 第 8 步 |

### 7.5 第三批(2026-09-21 续,即"第二轮")新增能力的索引

| 能力 | 关键符号 | 代码位置 | 本文的机制描述 |
|---|---|---|---|
| 描边外套(halo) | `theme::casing` | `theme.rs:517-523`;消费点 `app.rs:3222-3229`、`3284-3289`、`3332-3341`、`3397-3407`、`3419-3420`、`3432-3434`、`3443-3445`、`3465-3475` | §3.10(逐标注采样 + 外套 + 衬底)、§1.3(`theme.rs` 符号) |
| 文字衬底(scrim) | `theme::plate`、`theme::paint_label` | `theme.rs:526-532`、`557-570` | 同上 |
| 仅描边文字 | `theme::paint_text` | `theme.rs:536-549`;消费点 `draw_wheel_card`(`app.rs:5017-5024`) | §3.10(摇杆信息卡) |
| 自动对比 | `theme::auto_tone`、`theme::luma`、`Look::overlay_auto`(`default_true`)、左栏复选框 | `theme.rs:276-280`、`294-296`、`453-463`;`app.rs:3884-3900` | §3.10、§3.2b 第 13 步 |
| 亮度网格与逐处取档 | `app::LumaGrid`(`new`/`around`)、`PadApp.shot_lum`、`draw_overlay` 的 `tone_at` 闭包 | `app.rs:317-395`、`463-464`、`1739-1740`、`3187-3195` | §3.10、§3.2b 第 7 步 |
| 摇杆标注瘦身 | `quick_label`(只写「摇杆N / 临时摇杆N」) | `app.rs:3379-3386` | §3.10(摇杆信息卡) |
| 摇杆信息卡 | `PadApp.wheel_info`、`PadApp::wheel_at`、`draw_wheel_card`、`WHEEL_CLICK_MIN_RADIUS` | `app.rs:467-468`、`1743`、`3485-3488`、`4521-4540`、`4546-4574`、`4577-4578`、`4944-5030` | §3.10(摇杆信息卡) |
| 上升沿判定 | `rising_edge`;总开关键与 `Toggle` 模式启用键的判定 | `engine.rs:330-339`、`1071`、`1088-1089`、`1240-1251` | §3.6 第 8b / 9 / 12 步 |
| 引擎通知 | `notify`、`Shared.notices`、`pending_notice`、界面侧 `take` + `log` | `engine.rs:318-328`、`697-703`、`840-847`、`964-972`、`1094-1101`、`1204-1208`、`1445-1458`;`app.rs:1906-1919` | §3.6 第 2 / 9 / 11 / 14 步与 §3.6 ⑦、§3.2b 第 10b 步 |
| 空闲后坐标空间重查 | `Shared.space_recheck`、`last_key_at`、`pending_space_recheck`;界面侧 `space_rx = None` + `refresh_display_space()` | `engine.rs:704-710`、`846-847`、`1074-1086`;`app.rs:1920-1926` | §3.6 第 8c 步、§3.2b 第 10b / 11 步 |

---

## 8. 附录:测试与速查
**单元测试分布**(按 `#[test]` 实数 **43** 个,全部是纯逻辑、不需要设备):

| 文件 | 个数 | 测试 | 锁住的行为 |
|---|---|---|---|
| `engine.rs` | 13 | `aim_target_clamps_to_screen` / `aim_target_clamps_anchor_into_screen` / `aim_active_needs_anchor_and_gate` / `ctrl_alt_chord_fires_once_per_press` / **`rising_edge_ignores_key_auto_repeat`** / `released_blocks_aim` / `point_at_progress_is_arclength_based` / `fingers_enforces_concurrency_and_pairing` / **`fingers_respects_shared_device_pointer_pool`** / **`fingers_align_grows_only`** / **`signatures_track_structure_not_geometry`** / **`wheel_ownership_follows_engagement`** / **`held_key_resumes_its_normal_bind_the_moment_the_wheel_deactivates`** | 落点与锚点钳制、生效条件(锚点 + 门控)、Ctrl+Alt 一次组合只触发一次、**自动重复不算"刚按下"(`rising_edge(true, true) == false`,所以长按 F8 / 切换键不会连翻)**、交还鼠标后不瞄准、弧长插值、`Fingers` 并发与成对性、**共享触点池(轮盘/瞄准占掉的名额必须从普通绑定额度里扣)**、**补长不丢状态**、**结构指纹只认结构不认几何(改坐标不触发重建、换键码/增删条目必须触发)**、**归属判定(启用键永远归临时摇杆、未启用不占方向键)**、**用户实测场景:松开启用键的瞬间,仍按着的 K 必须立刻回到普通绑定** |
...
| `theme.rs` | 6 | **`look_overlay_tone_defaults_to_neutral`** / **`tone_changes_lightness_but_keeps_hue`** / **`tone_text_switches_for_dark_tone`** / **`tone_ring_fill_is_identity_at_zero`** / **`auto_tone_follows_background_luminance`** / **`casing_and_plate_are_opposite_to_ink`** | 老配置读入 `overlay_tone = 0`;档位只改明暗不改色相、填充透明度反向微调、`clamp_tone` 对 NaN/越界的收敛;加深档换近黑字;`tone == 0` 时逐字节不变(默认观感与旧版一致);**自动对比的方向(亮背景→加深、暗背景→变浅、中间调→接近 0、手动微调是叠加而非取代)**;**外套与衬底必须与本体反相**(同相就等于没加) |
...
| `app.rs` | 2 | **`luma_grid_samples_safely`** / **`remembered_scrcpy_location_survives_restart_and_move`** | **亮度网格:左右半黑半白能采出差异、越界/负坐标/1×1/0×0 都不 panic(它每帧、每个标注都会被查一次,panic 就是整个界面崩掉)**;启动定位三级兜底:记住目录能找回、记住 exe 能找回、**exe 与它的父目录都被删掉时靠再上一级的线索找回新版本**、位置彻底不存在时不误报 |

> **点数方法**:`Select-String -Path src\*.rs -Pattern '#\[test\]' | Group-Object Filename`(ripgrep 口径,UTF-8 安全)。本文上一版把总数写作 18,第二批点出 **39**,**第三批(第二轮)点出 43**:新增 `luma_grid_samples_safely`(`app.rs`)、`rising_edge_ignores_key_auto_repeat`(`engine.rs`)、`auto_tone_follows_background_luminance` 与 `casing_and_plate_are_opposite_to_ink`(`theme.rs`)。
>
> 需要提醒的是:**这一批新增的三处引擎行为里,只有"上升沿"有单测**——`notify` 的落账时机与 `space_recheck` 的"空闲 ≥3 秒"判定目前只能靠人工复现(它们都涉及 `Shared` 与时间),这也是 §8 速查表里"改引擎事件路径"一行的风险点。
...
| 改浮层绘制(圈、轨迹、锚点) | `app.rs::draw_overlay`、`draw_swipe_track`、`draw_easing_preview`(颜色取 `theme::Theme` 语义色 → `theme::tone_*` 过亮度档位 → **圈类元素再垫一层 `theme::casing`、文字走 `theme::paint_label`**) |
| **改浮层可读性(自动对比 / 衬底 / 外套)** | 三处要一起改:`theme.rs` 的 `auto_tone` / `luma` / `casing` / `plate` / `paint_label` / `paint_text`、`app.rs::LumaGrid`(采样口径)、`draw_overlay` 的 `tone_at` 闭包(谁按什么坐标取样)。**「档位从哪来」与「档位怎么用」分开看**:前者在 `tone_at`,后者在每个元素的 `tone_ring_fill` / `casing` 调用点 |
| **改"点一下摇杆看方向键"** | `app.rs::wheel_at`(命中半径与重叠取哪个)、`PadApp.wheel_info`(开关与清空时机)、`draw_wheel_card`(卡片内容与摆位)。**改内容要同时看 `ui_wheels` 里的"轮盘N"编号**(两处必须同一个编号) |
| **改引擎→界面的通知 / 坐标空间重查** | `engine.rs::notify` + `Shared.notices` / `Shared.space_recheck` + 三个写点(总开关键、触点池满、结构重建)与 `pending_notice` / `pending_space_recheck` 的落账处;界面侧在 `app.rs::ui` 的 10b 步。**持锁时只用 `notify(&mut g.notices, ..)` 借一个字段** |
| **改"按一下翻一次"的判定** | `engine.rs::rising_edge` + `Held`;总开关键(`engine.rs:1088`)与 `Toggle` 模式启用键(`engine.rs:1244`)两处消费点——**任何新的"切换型"按键都必须用它,否则会被自动重复连翻** |
