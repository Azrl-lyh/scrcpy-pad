...
>
> **行号快照**:本文核对时的规模为 `main.rs` 177、`app.rs` ≈4383、`engine.rs` ≈1197、`keymap.rs` 978、`capture.rs` 582、`adb.rs` ≈563、`theme.rs` 389、`control.rs` 173、`filedialog.rs` 65、`settings.rs` 258 行。这些文件在写作期间正被连续修改(接线陆续落地),因此行号仅作定位参考,**关键处优先给函数名**。本文已把同批落地的三处改动全部计入:`Wheel.scope`(§7.1)、`settings.rs` 及其接线(§3.2 / §7.2)、以及引擎的"关闭映射收尾"重构(`Shared.toolbar_release` + `EngineState` + `release_all`,见 §3.6 / §3.9)。
...
| `app.rs` | 4383 | 界面与"会话指挥":egui 全量 UI、启动序列、连接编排、配置/外观/设置的读写、取点与浮层绘制 | `adb`、`capture`、`control`、`engine`、`keymap`、`theme`、`filedialog`、`settings` | `main.rs`(`app::PadApp::new`);eframe 事件循环回调 `eframe::App::ui` |
| `engine.rs` | ≈1197 | 映射引擎线程:消费捕获事件、优先级判定、定时动作队列、瞄准子系统、把命令推进控制通道;并提供两条"关闭映射"路径共用的收尾实现 | `capture`、`control`、`keymap` | `app::PadApp::new` 里 `std::thread::spawn(|| engine::run(...))` |
| `keymap.rs` | 978 | 配置数据模型与几何/曲线算法:相对坐标 ↔ 像素的 `Mapper`、动作/轮盘/瞄准/外观的 serde 结构、滑动轨迹与缓动、旧配置升级 | `theme`(windows 下另有 → `capture`) | `app`(UI 与绘制)、`engine`(注入换算) |
| `capture.rs` | 582 | 全局输入捕获(平台无关接口 + 两套 `#[cfg]` 实现),产出统一的 `CaptureEvent` | 无(叶子) | `app`(创建/持有 `Capture`)、`engine`(只消费 `CaptureEvent` 类型) |
| `control.rs` | 173 | scrcpy 控制协议客户端:连接、dummy 握手、序列化 32/14 字节消息、专用写线程 + 丢弃式读线程 | 无(叶子) | `app`(`ControlClient::connect`)、`engine`(每次注入) |
| `adb.rs` | ≈563 | adb/scrcpy 的定位、查询与设备侧控制通道搭建;进程级 adb 路径静态变量 | 无(叶子) | `app`(定位/设备/截图/启动 scrcpy)、`main.rs`(`--selftest`) |
| `theme.rs` | 389 | 语义色、尺寸常量、配色/密度/背景图预设,以及 `apply_style` 把外观刷进 egui 样式 | 无(叶子) | `app`、`keymap`(`Profile.look` 字段类型) |
| `filedialog.rs` | 65 | 非阻塞原生文件对话框(独立线程 + channel 回传) | 无(叶子) | `app`(8 种对话框用途) |
| `settings.rs` | 258 | **程序级**设置持久化(scrcpy 三件套路径 / 启动参数 / 上次设备),与可分享的键位配置分离 | `app`(取配置目录,`crate::app::config_dir()`) | `app`(启动读取 + 帧末落盘) |
...
- `PadApp` 私有方法(按用途分组):
  会话:`new` / `log` / `serial` / `test_scrcpy` / `connect_control` / `disconnect` / `take_screenshot` / `refresh_devices` / `effective_adb` / `resync` / `sync_suite` / `refresh_display_space` / `sync_display_space` / `screen_size` / `stamp_profile_meta`
  ...
  程序级设置:`settings_snapshot` / `remember_now` / `persist_settings` / `open_config_dir`
...
- 自由函数:`profile_path` / `look_cache_path` / `load_look_cache` / `load_profile` / `read_profile_at` / `write_default_profile` / `fmt_timestamp` / `timestamp_compact` / `civil_from_days` / `set_circle_angle` / `circle_geometry` / `is_draft_slot` / `swipe_controls` / `draw_swipe_track` / `draw_easing_preview` / `install_cjk_font` / `egui_key_code`
- 私有类型:`StartPreset`、`ArgHelp`、`ArgEntry`、`KeySlot`、`CoordSlot`、`OverlayFilter`、`EasingEditTarget`、`DialogPurpose`、`DraftBind`(含 `Default`)
- 常量:`SCID = 0x1a2b3c4d`、`LOCAL_PORT = 28383`、`REPO_URL`、`AUTHOR`、`LICENSE_TEXT`、`BASE_SCRCPY_ARGS = "--stay-awake"`、`COORD_RANGE = -8192..=8192`

**`engine.rs`**
- `pub struct Shared { pub profile: Profile, pub enabled: bool, pub control: Option<ControlClient>, pub aim_live: AimLive, pub toolbar_release: bool }` —— `toolbar_release` 是"顶栏[映射: 开/关]按钮请求一次关闭收尾"的**请求位**(见 §3.6 ② 与 §3.9)
...
- 私有:`Fingers`(并发触点跟踪)、`AimState`、`SchedAct`、`WheelState`;函数 `bind_pid` / `wheel_pid` / `cancel_up_for` / `bind_point` / `point_at_progress` / `deactivate_wheel` / `release_conflicting_binds` / `update_wheel` / `aim_active` / `aim_lift` / `aim_release_local` / `aim_anchor` / `aim_target` / `aim_recenter` / `aim_on_motion` / `aim_tick` / `is_ctrl` / `is_alt` / `ctrl_alt_chord`
- 常量:`MAX_CONCURRENT_KEYS = 8`、`AIM_PID = 3000`、`KEY_LEFTCTRL/RIGHTCTRL/LEFTALT/RIGHTALT`、`IDLE_FAST_MS = 4`、`IDLE_SLOW_MS = 16`
...
**`adb.rs`**
- `set_adb_bin` / `adb_bin_now` / `adb_exe_name` / `list_devices` / `adb_version_at` / `screen_size` / `display_size` / `screencap_png` / `find_scrcpy` / `find_scrcpy_portable` / `scrcpy_exe_name` / **`find_adb_explicit`**(校验一条"记住的"adb 路径是否仍存在,存在即返回)/ `find_adb_in_dir` / `find_adb` / `find_server` / `scrcpy_version_at` / `device_info` / `start_control_server` / `launch_scrcpy`
...
- 私有:`static ADB_BIN: Mutex<Option<String>>`、`adb_bin`、`adb_cmd`、`find_in_path`、`find_scrcpy_portable_under`、`is_scrcpy_dir_name`、`program_base_dirs`、`find_file_recursive`、`parse_display_size`、`parse_size_token`
...
- `pub enum Preset` / `Density` / `BgFit`;`pub struct Look`(`theme`/`has_bg`/`clear_bg`)
- `pub fn apply_style`、`with_alpha`、`outline_text`、`outline_stroke`

**`filedialog.rs`**
- `pub struct FileDialogHandle` + `try_result() -> Option<Option<PathBuf>>`
- `pub fn pick_file()` / `save_file(default_name: &str)`
- 私有:`spawn_dialog`、`run_dialog`(Windows 同步 `rfd::FileDialog`;Linux `pollster::block_on(rfd::AsyncFileDialog)`)

**`settings.rs`**
- `pub struct Settings { remember_paths, scrcpy_path, server_path, adb_path, scrcpy_args, selected_serial, saved_at }` + `has_any_path` / `clear_paths` / `sanitize`
...
| 3 | 读配置 | `load_profile().unwrap_or_default()`;`profile_path()` 同时取出 | 从 `config_dir()/profile.json` 读;文件缺失或 JSON 损坏 → 用 `Profile::default()`(内置一个 WASD 永久轮盘 + F8 总开关键),不阻塞启动 |
| 4 | 外观缓存覆盖 | `if let Some(cached) = load_look_cache() { profile.look = cached }`,`app.rs:564` | `look.json` 是"程序自用"的外观缓存:即使从未点过[保存配置],重启后外观也保持上次调好的样子。缓存优先于配置里的 `look` |
| 5 | 构造 `Shared` | `Arc::new(Mutex::new(Shared{ profile, enabled:false, control:None, aim_live:Default }))`,`app.rs:568` | 这是 UI 与引擎之间**唯一的共享可变状态**。`enabled` 起步为 false(必须按总开关键才开打),`control` 起步为 `None` |
| 6 | 启动捕获层 | `Capture::start(cap_tx)`,`app.rs:576` | 成功 → `Some(Capture)`,失败 → `None` 并记下 `capture_err`(左栏会红字提示"输入捕获不可用")。随后克隆三个 `Arc<AtomicBool>`:`grab`、`mouse_grab`、`mouse_found`;捕获失败时用 `Arc::new(false.into())` 占位,保证后续代码不用到处判空 |
| 7 | 派生引擎线程 | `std::thread::spawn(move \|\| engine::run(shared, cap_rx, gui_tx, mouse_grab))`,`app.rs:597` | 顺序很关键:必须**先有捕获层(有事件源)、再有引擎(有消费者)**;引擎拿到 `cap_rx`、`gui_tx`、`mouse_grab` 后即进入自己的事件循环 |
| 8 | 自动寻找 scrcpy | 见下方"程序级设置插在哪"——先试记忆值,再 `adb::find_scrcpy()` → `adb::find_server(Some(&exe))` → `adb::scrcpy_version_at(&ps)`,`app.rs:616` 起 | 找 server 时**找不到就留空**:官方 Windows 发行包里 `scrcpy.exe`/`scrcpy-server`/`adb.exe` 同目录,留给 `sync_suite` 后续按同目录推导补齐,避免先塞一个相对路径反而堵死自动发现。同时生成 `found_msg` 供日志 |
| 9 | 定位 adb | 记忆值 → `adb::find_adb(Some(exe))` → `adb::set_adb_bin(...)`,`app.rs:662` | 顺序:记住的手动路径 → scrcpy 同目录 → PATH → 平台常见安装位置。**必须在列设备之前**:Windows 上 scrcpy 正常但 `adb` 不在 PATH 时,不这样做设备列表会是空的 |
| 10 | 列设备 | `adb::list_devices()`,`app.rs:683` | 解析 `adb devices`,只收状态为 `device` 的行(offline/unauthorized 不计);结果进 `devices: Vec<String>` |
| 11 | 构造结构体 | `let mut app = Self { ... }`,`app.rs:685` | 关键字段:`scrcpy_args` 取记忆值(空则 `BASE_SCRCPY_ARGS = --stay-awake`);`suite_synced` 被**故意置成空三元组**,从而在首帧 `sync_suite()` 时强制做一次完整联动补齐;新增 `settings_cache`、`remember_paths`、`applied_suite: None`;`overlay_filter = All`、`draft = DraftBind::default()`、撤销/重做栈为空、`pending_undo = None`、`undo_frame_marked = false`;`dialog_purpose` 给个默认值仅为满足类型 |
| 12 | 写启动日志 | `app.log(...)`,从 `app.rs:743` 起连写 3~4 条 | ① 使用顺序提示"连接手机 → [连接控制] → [启动 scrcpy] → 按总开关键开启映射";② `found_msg`(自动找到 / 使用上次记住的);③ 若开启了记住路径且确有路径,补一条"已读取记住的路径(<settings.json>)……";④ adb 路径(或"未找到 adb(设备列表将为空)")。日志上限 200 条(`log()` 内 `pop_front`) |
| 12b | 恢复上次设备 | `remembered.selected_serial` + `position()`,`app.rs:758` 起 | 记住的序列号若仍在 `devices` 里就选中它,日志"已选中上次使用的设备: …"——省得每次重连都在下拉框里挑 |
...
| # | 步骤 | 实际行为 |
|---|---|---|
| 6b | 读设置 | `let mut saved = settings::load().unwrap_or_default()` |
| 6c | 尊重"记住路径"开关 | `if !saved.remember_paths { saved.clear_paths() }` —— 关掉开关后**不采用**其中的路径,但仍保留启动参数一类的偏好;另存一份 `let remembered = saved.clone()` 供日志与 `remember_paths` 字段使用 |
| 6d | 校验路径 | `if saved.sanitize() && saved.remember_paths { eprintln!("[settings] 已丢弃不存在的旧路径: …") }` —— 死路径一律丢弃(留空才能走自动寻找) |
| 6e | 建缓存 | `SettingsCache::new(saved.clone())`(视为"已落盘",避免启动时立刻回写),并把 `saved.scrcpy_args` 单独取出备用 |
| 8′ | scrcpy 与 server | 记住的 `scrcpy_path` **且 `is_file()`** → 直接采用(日志写"已使用上次记住的 scrcpy: …(来自 <settings.json>)");否则 `adb::find_scrcpy()`。`server_path` 非空则直接用,否则 `adb::find_server(exe)`。版本仍由 `scrcpy_version_at` 现场取(只读、且能顺带发现路径失效) |
| 9′ | adb | 先 `adb::find_adb_explicit(saved.adb_path)`(只校验"记住的这条路径还在不在"),没有再走"scrcpy 同目录 → PATH → 平台常见位置";拿到就 `set_adb_bin` |
| 11′ | 字段初始化 | 新增字段 `settings_cache`、`remember_paths = remembered.remember_paths`、`applied_suite: None`;`scrcpy_args` 为"记忆值非空则用记忆值,否则 `BASE_SCRCPY_ARGS`" |
| 12′ | 设备与日志 | 若 `remembered.selected_serial` 非空且仍在 `devices` 里,按 `position()` 找到下标并选中,日志"已选中上次使用的设备: …";若开启记住路径且确有路径,再补一条"已读取记住的路径(<settings.json>),改动会自动保存;不想记住可在左栏取消勾选" |
...
| 2 | 路径联动 | `sync_suite()` | 三元组文本没变就直接返回;放在快捷键/界面之前,保证本帧用到的路径是补齐过的 |
...
| 7 | 回收截图 | `shot_rx.try_recv()` → `ctx.load_texture` → `shot = Some((tex,w,h))` → `sync_display_space(w,h)` | 截图尺寸就是"当前屏幕方向"下的真实坐标空间,顺手校正 |
| 8 | 回收文件对话框 | `dialog.try_result()` 拿到结果后按 `dialog_purpose` 分派:`ScrcpyExe` / `ServerJar` / `AdbExe`(写入路径 + `resync()` + `test_scrcpy()`,后两者不测)、`SaveLog` / `SaveProfileAs`(写文件)、`ChooseProfile`(读入并切换,清空撤销栈)、`NewProfile`(写默认配置 + 切换)、`PickBackground`(设背景并做可见性自动调整) | 每个分支都自带日志;`(_, None)` 统一记"已取消" |
...
| 13 | 左栏 `egui::Panel::left("left")` | 上部**可滚动的配置区**:"使用说明"折叠、捕获错误红字、[保存配置]/[另存为]/[重新加载]/[选用配置]/[新建配置]/[检测当前 json]/当前配置路径/[保存日志]、"外观"折叠(`ui_look`)、总开关键 + grab 复选框、"scrcpy 管理"([自动寻找全部]/[测试并刷新]、[记住路径] / [打开配置目录]、三个路径输入框 + [浏览]/[自动]、当前 adb 与 server 版本);下部日志区(`stick_to_bottom`,固定高度,倒序取最近 200 条) | 配置区高度用 `available_height() - 150` 预留日志区,日志因此不会被顶出屏幕 |
...
| 17 | 帧末:设置落盘 | `remember_now()` + `persist_settings()` | 与撤销点同批收尾;`save_if_dirty` 内部做内容比较,所以每帧调用无副作用 |
...
flowchart LR
    ...
    subgraph ENG["引擎线程(engine::run)"]
        E["事件循环 + scheduled 定时队列<br/>Fingers / WheelState / AimState 均为线程局部"]
    end
    ...
    U -- "Arc&lt;Mutex&lt;Shared&gt;&gt;<br/>写 profile / control<br/>读 enabled / aim_live" --> E
...
| `SharedState` | `Arc<Mutex<Shared>>` | 双向 | `profile`(UI 写、引擎只读并 clone 小结构)、`enabled`(引擎翻转、UI 显示与可点击)、`control`(UI 写、引擎借用)、`aim_live`(引擎写、UI 读来诊断显示)、`toolbar_release`(UI 置位请求关闭收尾、引擎取走并复位) |
...
2. **瞄准维护 + 同步鼠标捕获**:在锁内 clone 一份 `Aim`(小结构),算 `mouse_grab` 的组合布尔值并 `store`;在线且已连接时 `aim_tick`(映射关/门控松 → 抬指;静止归中 → 回锚点),否则 `aim_release_local` 只清本地状态;最后把 `aim.ox/oy/down/active/sent` 抄进 `Shared.aim_live` 供 UI 显示。
...
4. **Motion 分支**:更新 `aim_live` 计数/最近位移;开着映射且有在线通道时,`profile.mapper(...)` + `aim_on_motion`(累积偏移 → 落点钳制 → `touch_down/move` → 按策略归中),然后 `continue` —— 鼠标位移**不参与按键绑定流程**。
...
9. **总开关键(最高优先级,任何时候都生效)**:翻转 `enabled`;**关闭瞬间**走与顶栏按钮完全相同的 `release_all`(`EngineState` 借用同一批状态):`aim_lift` 抬瞄准触点、遍历所有绑定 `fingers.release` 并按 `bind_point` 抬触点、`active_android_keys.drain()` 补发 `key(false)`、落下的轮盘先 `touch_move` 回圆心再 `touch_up`,最后 `fingers.free_all()` + 所有轮盘状态(含临时轮盘的 `active`)清零;没有控制通道时走 `None` 分支,只清本地状态(否则重开打时状态会残留)。**重新开启**时把 `aim.released` 复位(避免上一局的"交还鼠标"带过来);若此刻按钮也正好请求了收尾,则视为已一并处理(`toolbar_release = false`),避免下一轮再收一次。随后 `continue`。
10. **未开启 / 未连接直接跳过**:`if !enabled { continue }`;`let Some(ctl) = g.control.as_ref() else { continue }`;`if !ctl.is_connected() { continue }`。
11. **取换算器与对齐状态**:`let m = profile.mapper((ctl.screen_w, ctl.screen_h))` 是**唯一换算入口**;轮盘数量变化时重建 `wheels: Vec<WheelState>`;`fingers.align(binds.len())`(只增不减,删除绑定产生的残留由 `free_all` 兜底)。
12. **临时轮盘启用键**:命中 `w.temp.key` 时,`Hold` 模式按下即 `active = true` 并 `release_conflicting_binds`(把与该轮盘四个方向键同键位的普通绑定/系统键先释放,免得方向输入被抢占、触点卡死),松开即 `deactivate_wheel`;`Toggle` 模式按下翻转。**命中即 `consumed = true` 并 `continue`** —— 启用键不会同时触发自己的其它绑定。
13. **轮盘方向键**:只处理"生效中"的轮盘(`temp.is_none()` 或 `active`);命中四向之一就记 `pressed[d]` 并 `update_wheel`,**`handled = true` 并 `continue`**。
14. **普通绑定**:遍历 `profile.binds`,匹配 `bind.key == ev.code` 后按动作类型分派(下一节)。
...
- 首次按下:`touch_down(圆心)` 然后 `touch_move(目标)`(先落圆心再推,符合摇杆手感);
...
- 触发:每轮循环顶部(第 1 步)处理所有 `t <= now` 的动作。等待时长取"最近到期时刻"与 4ms 的较小值,所以点按的抬起/滑动的每一步能落在预定时刻(注释:误差由 ~4ms 降到 ~1ms);没有计划动作时退回 4ms 轮询,保证瞄准维护与鼠标捕获状态及时刷新。
...
同时按下的普通键受 `MAX_CONCURRENT_KEYS = 8` 限制(设备端 `PointersState.MAX_POINTERS = 10`,瞄准/轮盘指针另计,留出余量),`Fingers::try_down` 在"已按下"或"并发已满"时返回 false,调用方就不注入 `DOWN`;`release` 只在确有按下记录时返回 true,绝不误抬别的键。
...
| `settings.json` | 启动时 `settings::load()`(见 §3.2) | 每帧末 `remember_now()` 登记 + `persist_settings()` 落盘:内容与上次写出的完全一致则跳过;勾选关掉"记住路径"后两者都直接返回(旧文件保留但不再读写);写失败只记一条日志,不影响运行 | 路径/启动参数/选中设备的改动 |
...
- **映射关闭时的收尾优先级**:两条关闭路径(总开关键与顶栏按钮)都走 `engine::release_all`,都是**先抬触点再清状态**(见 §3.6 第 1b / 9 步),避免留下"手指按在屏幕上"的僵尸触点;`fingers.free_all()` 与轮盘状态清零是最后的兜底。**注意顶栏按钮只能置请求位**:触点的"已按下"记录是引擎线程的局部状态(`Fingers` / `WheelState` / `AimState`),界面线程看不到也改不了。
...
| 文件对话框 | `pollster::block_on(rfd::AsyncFileDialog)`(xdg-portal,zbus 纯 Rust,无 GTK 构建依赖) | 同步 `rfd::FileDialog`(Win32 原生) | `pick_file()` / `save_file()` + `FileDialogHandle::try_result()`(始终在独立线程里跑,UI 不卡) |
...
> 这三处改动与本文同批写入代码,写作时均已落地并逐行核对(`Wheel.scope`、`settings.rs` 及其在 app/main 的接线、引擎的"关闭映射收尾"重构)。
...
| 消费点 | `engine.rs::update_wheel` 已改为 `let wr = w.push_px(m);`(engine.rs:936,注释保留"半径 × 影响范围"的推导),斜向仍用 `1/√2` 归一化后乘 `wr`;凡"按方向后推多远"的计算都应走 `push_px`,避免某处漏乘导致界面圈与手感不符 |
| 兼容与测试 | `keymap.rs` 新增 `wheel_scope_defaults_and_clamps`(老 json 无该字段 → 1.0;`clamp_scope` 对 0/负数/99/NaN 的收敛)与 `wheel_push_px_is_a_radius_scale`(scope 缩放与换屏幕尺寸的同比例性);`Profile::default()` 的默认轮盘显式写 `scope: DEFAULT_WHEEL_SCOPE` |
...
| 保存内容 | `scrcpy_path` / `server_path` / `adb_path` / `scrcpy_args` / `selected_serial`(上次设备,重连同一台手机时优先选中),外加 `remember_paths`(默认 true)与 `saved_at`(epoch 秒,仅供查看) |
| 启动读取(实际代码) | `let mut saved = settings::load().unwrap_or_default();` ① 若 `!saved.remember_paths` → `saved.clear_paths()`(不采用路径,但保留启动参数等偏好);② `let remembered = saved.clone()`(供日志与字段初始化);③ `if saved.sanitize() && saved.remember_paths { eprintln!("[settings] 已丢弃不存在的旧路径: …") }`;④ 取出 `saved_args` 并 `SettingsCache::new(saved.clone())` |
| 怎么"跳过自动寻找" | scrcpy:记住的路径 **且 `is_file()`** 才直接用(日志"已使用上次记住的 scrcpy: …(来自 <settings.json>)"),否则回落 `adb::find_scrcpy()`;server:字段非空直接用,否则 `find_server`;adb:`adb::find_adb_explicit(saved.adb_path)`(只校验这条路径还在不在)→ `find_adb` → PATH → 平台常见位置。**版本号仍现场查一次**(`scrcpy_version_at`),只读且能顺带发现路径失效 |
| 失效回退 | `Settings::sanitize()` 去掉首尾空白并**清空不存在的文件路径**,返回是否有改动。理由写在注释里:死路径比空路径更糟 —— 空路径还能走"同目录推导 + PATH"的自动逻辑,死路径会让 `[启动 scrcpy]` 直接失败 |
| 写盘策略(UI 侧方法) | `settings_snapshot()` 把界面上的路径/参数/选中设备打成一 `Settings`;`remember_now()` 在 `remember_paths` 为真时 `mark_dirty(settings::stamped(..))`(登记,不写盘);帧末 `persist_settings()` 调 `SettingsCache::save_if_dirty()` —— 内容与上次写出的完全一致直接返回 `None`,所以每帧调用也不会反复写盘;成功记"设置已保存: <路径>",失败记一条并说明"本次改动只在本次运行内有效"。`SettingsCache::new(loaded)` 把刚读出的内容当作"已落盘",避免启动时立刻回写 |
| 界面入口(左栏 scrcpy 管理区) | `ui.checkbox(&mut self.remember_paths, "记住路径(下次启动直接用)")`(hover 说明会告诉你它存到 `settings.json`);勾上时立即 `remember_now()`,取消时日志提示"不再写入 settings.json(旧内容保留但不再读取)"。旁边 `small_button("打开配置目录")` → `open_config_dir()`:`create_dir_all` 后用 `explorer`(Windows)/`open`(macOS)/`xdg-open`(其它 unix)打开 `config_dir()`,失败则把目录路径打进日志让用户手开 |
| 辅助方法 | `has_any_path()`(启动日志/界面提示"已记住")、`clear_paths()`(清空三个路径与序列号,保留 `remember_paths` 与启动参数) |
| 测试 | `legacy_settings_json_still_loads`(缺字段 json 可读、`remember_paths` 缺省 true)、`sanitize_drops_nonexistent_paths`(死路径清空、序列号去空白、已干净时不报改动)、`cache_skips_identical_content`、`path_helpers` |
| 依赖方向与实际接线 | `settings → app::config_dir()`。`app.rs` 已把配置目录收敛为唯一的 `pub fn config_dir() -> PathBuf`(app.rs:4028:`ProjectDirs` → exe 同级目录(便携模式)→ `.`),`profile_path()` / `look_cache_path()` 均改为由它派生;`main.rs:8` 增加 `mod settings;`。`PadApp` 新增字段:`settings_cache: SettingsCache`、`remember_paths: bool`、`applied_suite: Option<(String,String,String)>`(三元组一变就 `refresh_devices()`,替代了原先"按生效 adb 是否变化"的判定) |
...
## 8. 附录:测试与速查
**单元测试分布**(共 18 个 `#[test]`,全部是纯逻辑、不需要设备):

| 文件 | 测试 | 锁住的行为 |
|---|---|---|
| `adb.rs` | `find_scrcpy_portable_in_sibling_and_nested_dirs` / `ignore_dirs_not_starting_with_scrcpy` / `parse_display_size_prefers_current_orientation` | 便携目录两种摆放都能找到、非 scrcpy 目录不误认、`cur=` 优先于 `init=`(横屏坐标空间) |
| `keymap.rs` | `zoom_radius_actually_changes` / `upgrade_refuses_mismatched_orientation` / `upgrade_profile_keeps_pixel_positions` / `legacy_json_still_loads_as_pixels` / `wheel_scope_defaults_and_clamps` / `wheel_push_px_is_a_radius_scale` | 半径缩放真的生效(曾经的"绝对差判默认值"bug)、方向不匹配时拒绝升级、升级不改像素位置、旧 json 仍可读、scope 缺省与收敛 |
| `engine.rs` | `aim_target_clamps_to_screen` / `aim_target_clamps_anchor_into_screen` / `aim_active_needs_anchor_and_gate` / `ctrl_alt_chord_fires_once_per_press` / `released_blocks_aim` | 落点与锚点钳制、生效条件(锚点 + 门控)、Ctrl+Alt 一次组合只触发一次、交还鼠标后不瞄准 |
| `settings.rs` | `legacy_settings_json_still_loads` / `sanitize_drops_nonexistent_paths` / `cache_skips_identical_content` / `path_helpers` | 老设置可读、死路径清理、缓存不重复写盘 |
...
| 改浮层绘制(圈、轨迹、锚点) | `app.rs::draw_overlay`、`draw_swipe_track`、`draw_easing_preview`(颜色一律取 `theme::Theme` 语义色) |
| 改配色/密度/背景图 | `theme.rs`(`Theme` 的四个预设、`Preset::panel/window`、`apply_style`、`Look`) |
| 改坐标语义 / 配置格式 | `keymap.rs`(`PROFILE_VERSION`、`CoordUnit`、`Mapper`、`upgrade_profile`)——**改这里必须同时想清楚旧配置升级路径** |
| 加"程序级(本机私有)"的设置项 | `settings.rs::Settings` + `app.rs` 的 `SettingsCache` 接线 |
